...
function fmtPrice(val) {
  // Prisma Decimal bisa string; amankan ke number untuk format locale
...
function ProductCard({ p, index, isVisible }) {
  ...
  const img = p.imageUrl || p.image || "/images/placeholder.png"
  ...
  const handleBuyNow = async (product, qty = 1) => {
    ...
    try {
      sessionStorage.setItem("buyNowItem", JSON.stringify({
        productId: product.id, qty, name: product.name, price: product.price
      }))
    } catch {}
  ...
  const handleAddToCart = (product) => {
    setSelectedProduct(product); setModalType("cart"); setQuantity(1); setShowModal(true)
  }
  const handleBuyClick = (product) => {
    setSelectedProduct(product); setModalType("buy"); setQuantity(1); setShowModal(true)
  }
  const closeModal = () => { setShowModal(false); setSelectedProduct(null); setModalType(""); setQuantity(1) }
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 flex flex-col text-center">
          ...
          <p className="text-amber-700 font-bold text-lg mb-2">Rp {fmtPrice(p.price)}</p>
          ...
          <div className="flex gap-2 w-full mt-2">
            <button
              onClick={(e) => {
                e.stopPropagation()
                if (!isLoggedIn) { alert("❌ Login terlebih dahulu untuk menambahkan ke keranjang"); window.location.href = "/auth/login"; return }
                handleAddToCart(p)
              }}
              disabled={soldOut}
            ...
            <button
              onClick={(e) => {
                e.stopPropagation()
                if (!isLoggedIn) { alert("❌ Login terlebih dahulu untuk membeli produk"); window.location.href = "/auth/login"; return }
                handleBuyClick(p)
              }}
              disabled={soldOut}
      ...
      {showModal && selectedProduct && (
        <div className="fixed inset-0 bg-transparent flex items-center justify-center p-4 z-50">
          <div className="absolute inset-0 bg-black/10 backdrop-blur-[1px]" onClick={closeModal} />
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 relative border-4 border-amber-300 z-10">
            <button onClick={closeModal} className="absolute top-3 right-3 text-gray-500 hover:text-red-600 bg-gray-100 rounded-full p-1 transition">✕</button>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div>
                ...
                <div className="mt-4 p-4 bg-amber-50 rounded-xl border border-amber-200">
                  ...
                  <p className="text-gray-700 text-sm leading-relaxed text-center">{desc}</p>
                </div>
                ...
                <div className="mt-4 p-4 bg-white rounded-xl border border-amber-200">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-3">
                      <button onClick={() => setQuantity(q => Math.max(1, q - 1))} disabled={quantity <= 1} className="w-8 h-8 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed hover:bg-amber-200 transition">
                        <Minus className="w-4 h-4" />
                      </button>
                      <span className="text-lg font-bold text-amber-800 min-w-8 text-center">{quantity}</span>
                      <button onClick={() => setQuantity(q => Math.min((selectedProduct.stock ?? 0), q + 1))} disabled={quantity >= (selectedProduct.stock ?? 0)} className="w-8 h-8 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed hover:bg-amber-200 transition">
                  ...
                  <div className="flex justify-between items-center mt-3">
                    ...
                    <div className="text-right">
                      <span className="text-xl font-bold text-amber-700">Rp {fmtPrice((Number(selectedProduct.price) || 0) * quantity)}</span>
                    </div>
                  </div>
                </div>
...
export default function BestSellers({ title = "Best Sellers" }) {
  ...
  // Load best sellers dari BE (bukan array lokal)
  useEffect(() => {
    let stop = false
    ;(async () => {
      try {
        // Ambil semua produk + flag isBestSeller + sold dari BE
        // (server sudah menghitung berdasarkan order_items 30 hari terakhir)
        const res = await fetch("/api/products?days=30&top=8", { cache: "no-store" })
        const rows = await res.json()
        const list = Array.isArray(rows) ? rows : []

        // Ambil hanya yang best seller, urutkan by sold desc, ambil maks 8
        const best = list
          .filter(p => !!p.isBestSeller)
          .sort((a, b) => (Number(b.sold || 0) - Number(a.sold || 0)))
          .slice(0, 8)

        if (!stop) setItems(best)
      } catch (e) {
  ...
  // Reveal on scroll
  useEffect(() => {
    if (prefersReduce) { setIsVisible(true); return }
  ...
  return (
    <section id="best" ref={sectionRef} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-10 pb-16 relative">
      ...
      <div className={`relative mb-8 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-3 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
        <div>
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-amber-900 drop-shadow-sm leading-snug">{title}</h2>
          <p className="mt-1 text-sm sm:text-base text-amber-800/80">Pilihan kopi terlaris, disukai pelanggan setia kami ☕</p>
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5 sm:gap-6">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-64 sm:h-72 bg-gradient-to-b from-amber-100/60 to-amber-50 animate-pulse rounded-2xl border border-amber-100 shadow-sm">
        ...
        <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5 sm:gap-6 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
          {items.map((p, idx) => (
